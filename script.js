...
window.addEventListener('error', e => {
  DEBUG.error('window.error', e.message, '@', e.filename, e.lineno + ':' + e.colno);
});
...
function initCake() {
  ...
  function phys(color, rough, metal, emit = 0x000000, emitInt = 0, cc = 0) {
    return new THREE.MeshPhysicalMaterial({
      ...
      emissive: emit, emissiveIntensity: emitInt,
  ...
  [[2.1, 0.14, 0.72], [1.5, 0.11, 2.29], [0.95, 0.09, 3.61]].forEach(([r, tube, y]) => {
    const t = new THREE.Mesh(new THREE.TorusGeometry(r, tube, 10, 48), icingMat.clone());
  ...
  const spkColors = [0xff69b4, 0xffd700, 0x00ffcc, 0xff6347, 0x9b59b6, 0xffffff];
  for (let i = 0; i < 55; i++) {
  ...
  const candleHues = [0xff69b4, 0xffd700, 0x9b59b6, 0x00ffcc, 0xff4444, 0xffd700];
  ...
  function blowCandle(i) {
    ...
    gsap.to(flameGroups[i].scale, { x: 0, y: 0, z: 0, duration: 0.35, ease: 'back.in(2)' });
  ...
  function tryBlowAt(nc) {
    ...
    const entry = flameMeshes.find(f => f.mesh1 === hitMesh || f.mesh2 === hitMesh);
  ...
  function animate() {
    ...
    flameGroups.forEach((fg, i) => {
      ...
      candleLights[i].intensity = 0.8 + Math.sin(t * 12 + seed) * 0.4 + bass * 0.6;
    });
...
function initMusicPlayer() {
  ...
  audioEl.addEventListener('canplaythrough', () => DEBUG.log('audio: canplaythrough'));
  audioEl.addEventListener('waiting', () => DEBUG.warn('audio: waiting (buffering)'));
  audioEl.addEventListener('stalled', () => DEBUG.warn('audio: stalled'));
  audioEl.addEventListener('error', () => {
    const mediaErr = audioEl.error;
    DEBUG.error('audio: error', mediaErr ? { code: mediaErr.code, message: mediaErr.message } : 'unknown');
  });
  ...
  function play() {
    ...
    audioEl.play().then(() => {
      ...
      if (err && err.name === 'AbortError') {
        DEBUG.warn('audio: play interrupted (AbortError), usually harmless during quick play/pause toggles');
      } else {
  ...
  audioEl.addEventListener('ended', () => {
    if (repeat) { audioEl.currentTime = 0; play(); }
    else nextTrack();
  });
  ...
  function drawViz() {
    ...
    if (!analyser || !isPlaying) {
      ...
      vizCtx.beginPath();
      for (let x = 0; x < vW; x++) {
        const y = vH / 2 + Math.sin((x / vW) * Math.PI * 4 + t * 2) * 6 * Math.sin(t * 0.5);
...
function confettiBurst(n) {
  const cols = ['#ff69b4','#ffd700','#9b59b6','#00ffcc','#ff4444','#ffffff'];
...
function initGSAP() {
  ...
  // ── Hero entrance ─────────────────────────────────────────
  gsap.to('.hero-eye',   { opacity: 1, y: 0, duration: 0.9, ease: 'power2.out', delay: 0.5 });
  gsap.to('.hero-title', {
    opacity: 1, scale: 1, duration: 1.1, ease: 'back.out(1.5)', delay: 0.8,
    onStart() { gsap.set('.hero-title', { scale: 0.5 }); }
  });
  gsap.to('.hero-sub',   { opacity: 1, y: 0, duration: 0.9, ease: 'power2.out', delay: 1.3,
    onStart() { gsap.set('.hero-sub', { y: 30 }); }
  });
  gsap.to('.hero-badge', { opacity: 1, scale: 1, duration: 0.7, ease: 'back.out(2)', delay: 1.8,
    onStart() { gsap.set('.hero-badge', { scale: 0 }); }
  });
  ...
  ScrollTrigger.create({
    ...
    onEnter() {
      gsap.to('#jojo-arrow', {
        ...
        duration: 2,
        ease: 'power2.inOut',
        onStart() { gsap.set('#jojo-arrow', { x: -window.innerWidth - 300 }); }
      });
      ...
      gsap.from('.footer-body > *', {
        y: 30, opacity: 0, stagger: 0.18, duration: 0.8,
...
window.addEventListener('load', () => {
  ...
  const loaderSafetyTO = setTimeout(() => {
    DEBUG.warn('Loader safety timeout hit (>12s). Forcing loader hide so UI is usable.');
    hideLoader();
  }, 12000);
